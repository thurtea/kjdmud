...
That said, kjdmud did not invent the LPC language or its surrounding driver
...
existing LPC mudlib code has a reasonable chance of running on kjdmud
...
- **DGD** (Dworkin's Game Driver): an LPC-family driver with a distinct
  ...
  functions) that kjdmud draws on for comparison and, in a few specific
  places, real feature support.

None of these projects are affiliated with kjdmud, endorse it, or are
responsible for it. kjdmud is not a fork, wrapper, or compatible
...
For a detailed, source-cited breakdown of exactly which behaviors kjdmud
