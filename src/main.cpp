...
#include "kjdmud/dialect/InaugurateMasterBoot.hpp"

namespace {
void handleSignal(int) {
    kjdmud::Scheduler::requestShutdown();
}
}
...
int main(int argc, char** argv) {
    ...
    } else if (const char* envVal = std::getenv("KJDMUD_MAX_ITERATIONS")) {
    ...
    kjdmud::registerCoreEfuns();

    kjdmud::ObjectManager objectManager(config);
    kjdmud::VM vm(objectManager, config);
    ...
    objectManager.setEfunExistsChecker([](const std::string& name) {
        return kjdmud::EfunTable::instance().exists(name);
    });

    kjdmud::Scheduler scheduler(vm);
    vm.setScheduler(&scheduler);
    kjdmud::Server server(config, vm, objectManager, scheduler);

    std::cout << "kjdmud booting...\n";
    ...
    auto bootApi = kjdmud::makeBootApiForConfig(config);
    if (auto uid = kjdmud::queryMasterUid(vm, *bootApi)) {
    ...
    kjdmud::applyInaugurateMaster(vm, *bootApi);
