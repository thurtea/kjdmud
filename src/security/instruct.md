...
kjdmud's goal: **one security model that covers all three dialects**, with
the dialect's boot API mapping its own permission callbacks to the shared
enforcement layer.

## Files to create

### `include/kjdmud/security/SecurityManager.hpp`
...
target_link_libraries(kjdmud PRIVATE security)
```
...
- Security must be a compile-time option (`KJDMUD_ENABLE_SECURITY`) that
