...
LPC driver has this. It makes kjdmud the first LPC runtime with a
...
```bash
build/kjdmud --lsp etc/driver.cfg
```
...
### `include/kjdmud/lsp/LspServer.hpp`
...
### `include/kjdmud/lsp/SymbolIndex.hpp`
...
```cmake
option(KJDMUD_ENABLE_LSP "Build LSP server" ON)
if(KJDMUD_ENABLE_LSP)
    ...
    target_link_libraries(kjdmud PRIVATE lsp)
endif()
```
