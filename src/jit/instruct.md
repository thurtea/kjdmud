...
kjdmud is the first LPC driver to offer LLVM-quality codegen as a
...
### `include/kjdmud/jit/JitCompiler.hpp`
...
`kjdmud_tag(Value*)`, `kjdmud_as_int(Value*)`,
`kjdmud_as_double(Value*)`, `kjdmud_box_int(i64)`,
`kjdmud_box_double(double)`.
...
```cmake
option(KJDMUD_ENABLE_JIT "Enable LLVM JIT backend" OFF)
if(KJDMUD_ENABLE_JIT)
    ...
    target_compile_definitions(jit PRIVATE ${LLVM_DEFINITIONS} KJDMUD_JIT_ENABLED)
    target_link_libraries(jit PUBLIC vm ${llvm_libs})

    target_link_libraries(kjdmud PRIVATE jit)
endif()
```
