...
class OriginGuard {
public:
    OriginGuard(kjdmud::VM& vm, kjdmud::Origin origin) : vm_(vm) { vm_.pushOrigin(origin); }
...
public:
    CommandGiverGuard(kjdmud::VM& vm, const std::shared_ptr<kjdmud::LpcObject>& ob) : vm_(vm) {
...
bool asArithmeticOperand(const kjdmud::Value& v, double& out) {
...
// these three kinds.
std::string formatNumberForConcat(const kjdmud::Value& v) {
    if (auto* i = std::get_if<int64_t>(&v.data)) {
...
// "%s", "%d"/"%i", "%x"/"%X", "%o", "%f", "%(regexp)", the "%*" skip
// modifier, and "%s" directly adjacent to another specifier with no
...
SscanfOutcome runSscanf(const std::string& in0, const std::string& fmt0, size_t maxAssigns) {
    ...
    for (;;) {
        ...
        if (spec == 'd' || spec == 'i' || spec == 'x' || spec == 'X' || spec == 'o') {
            // Real inter_sscanf(): 'd' decimal, 'x'/'X' hex, 'o' octal.
            // '%i' matches '%d' here (signed decimal via strtoll base 10).
            int base = 10;
            if (spec == 'x' || spec == 'X') base = 16;
            else if (spec == 'o') base = 8;
            const char* start = in0.c_str() + ip;
            char* endPtr = nullptr;
            long long value = std::strtoll(start, &endPtr, base);
        ...
        if (spec != 's') {
            throw NotImplementedError(
                ...
                "' (only %s, %d, %i, %x, %X, %o, %f, %%, %(regexp) are supported)");
        }
