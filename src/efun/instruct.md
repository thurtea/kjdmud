...
- `include/kjdmud/vm/Value.hpp` - every efun signature is `Value(VM&, vector<Value>&)`
...
| 2 | `call_stack` | 3 | 1 | Implemented: modes 0 (filenames), 1 (objects), 2 (function names), 3 (origins). Mode 2 tracks names via ObjectFrameGuard in lockstep with callStack_. Mode 3 zip-aligns originStack_ from the innermost frame. |
