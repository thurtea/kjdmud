...
- `libssl` and `libcrypto` development headers (TLS on listen ports,
  2026-09-05): `src/net/CMakeLists.txt` uses
  `pkg_check_modules(LIBSSL REQUIRED libssl)` and
  `pkg_check_modules(LIBCRYPTO REQUIRED libcrypto)`. `src/efun` already
  linked libcrypto for `hash()`.
...
pkg-config file; `openssl-devel` provides libssl/libcrypto.)
...
- The real keys, confirmed against `Config.cpp`'s own parser (every one
  ...
  `simul_efun_file`, `global_include_file`, `dialect`, plus `listen`,
  `tls_cert`, and `tls_key` (2026-09-05).
- `etc/driver.cfg` uses even ports from 1122: telnet 1122, websocket
  1124, telnet TLS 1126, websocket TLS 1128. `port: 1122` remains the
  telnet default. See `summary.md` for Mudlet and browser connect
  steps.
