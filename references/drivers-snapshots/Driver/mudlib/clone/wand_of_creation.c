...
static int may_use() {
    ...
    if (!this_player() || !wizardp(this_player())) {
        write("The rod does not answer you.\n");
        return 0;
    }
    return 1;
}
...
int cmd_create(string str) {
    ...
    body = "// requisitioned into being by the reeve's rod\n"
        ...
        "void create() {\n"
        "    set_short(\"" + str + "\");\n"
        "    set_long(\"" + str + ", freshly cobbled together by the reeve's rod.\\n\");\n"
        "    set_ids(({ \"" + str + "\" }));\n"
        "}\n";
...
int cmd_edit(string str) {
    ...
    if (sscanf(file_name(ob), "/data/created/%s", leaf) != 1) {
        write("Only requisitioned objects can be edited.\n");
        return 1;
    }
...
int cmd_room(string str) {
    ...
    fname = replace_string(str, " ", "_");
    path = "/data/created/" + fname;
    body = "// room requisitioned by the reeve's rod\n"
