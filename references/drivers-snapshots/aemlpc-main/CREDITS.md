...
aemlpc is a from-scratch LPC game driver. Its lexer, parser, code generator,
...
That said, aemlpc did not invent the LPC language or its surrounding driver
...
existing LPC mudlib code has a reasonable chance of running on aemlpc
...
- **DGD** (Dworkin's Game Driver): an LPC-family driver with a distinct
  ...
  functions) that aemlpc draws on for comparison and, in a few specific
  places, real feature support.

None of these projects are affiliated with aemlpc, endorse it, or are
responsible for it. aemlpc is not a fork, wrapper, or compatible
...
For a detailed, source-cited breakdown of exactly which behaviors aemlpc
