...
Establish the current aemlpc baseline with a clean manual build, then
boot and exercise external LPC mudlibs as compatibility probes.

## Current checkpoint

- Live repository: `/home/thurtea/aemlpc`, `main` at `f057c53`.
- Live remote: `https://github.com/thurtea/aemlpc`.
- Backup snapshots: AMLP at `89741e4`; crysis at `f814bfc` and one local
  commit ahead of its remote.
- Current driver binary: `build/aemlpc`. Test target:
  `build/test/aemlpc_tests`.
...
bundled `mudlib/`. Each test uses a minimal aemlpc adapter config, not
