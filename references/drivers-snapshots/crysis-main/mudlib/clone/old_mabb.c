...
int cmd_talk(string arg) {
    ...
    if (strsrch(topic, "wand") != -1 || strsrch(topic, "creation") != -1
        || strsrch(topic, "rod") != -1) {
        write("\"That old wand on the worktable downstairs? Been there "
            "longer than I have. Nobody's claimed it in years -- reckon "
            "it's yours now if you're fool enough to carry it.\"\n");
        return 1;
    }
