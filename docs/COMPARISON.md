...
| 2.32a | Done | GMCP/MSDP/MSSP telnet protocol extension efuns. GMCP/MSDP now also fire their real `gmcp_enable()`/`msdp_enable()` applies on negotiation (`Server::fireGmcpEnableIfNegotiated()`/`fireMsdpEnableIfNegotiated()`, same shape as MSP's own `msp_enable()`), and the MSDP send efun is `send_msdp_variable` (its real name; this row's own earlier `send_msdp` was a naming defect, corrected), see `docs/dev/STATUS.md` 2026-09-22 |
...
abstraction (`include/kjdmud/dialect/BootApi.hpp`) currently recognizes
