...
Manually establish a clean kjdmud build baseline, then test external LPC
mudlibs as separate compatibility probes. Do not copy external code into
the bundled `mudlib/`.

## Current status

- Canonical repository: `/home/thurtea/kjdmud`, branch `main`, commit
  `f057c53`, remote `https://github.com/thurtea/kjdmud`.
- Historical backups:
...
- The kjdmud rename is complete for the C++ namespace, include directory,
  executable (`build/kjdmud`), and test executable
  (`build/test/kjdmud_tests`). Some legacy AMLP names intentionally remain
...
```bash
cd /home/thurtea/kjdmud
rm -rf build
cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug -DKJDMUD_BUILD_TESTS=ON
...
sed "s|^mudlib_root:.*|mudlib_root: $TMI_ROOT|" etc/driver_tmi2.cfg > /tmp/kjdmud-tmi2.cfg
./build/kjdmud /tmp/kjdmud-tmi2.cfg 3
```
...
```bash
cd /home/thurtea/kjdmud
./build/kjdmud /tmp/kjdmud-tmi2.cfg
```
...
printf '%s\n' \
  ...
  'max_string_length: 2000000' > /tmp/kjdmud-riftsmud.cfg
./build/kjdmud /tmp/kjdmud-riftsmud.cfg 3
```
...
printf '%s\n' \
  ...
  'max_string_length: 256000' > /tmp/kjdmud-nightmare.cfg
./build/kjdmud /tmp/kjdmud-nightmare.cfg 3
```
